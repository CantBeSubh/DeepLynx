...
public interface IEventBusiness
{
    ...
    Task<List<EventResponseDto>> QueryEventsByUserSubscriptions(long organizationId, long userId, EventsQueryRequestDTO? filterDto, long? projectId);
    Task<EventResponseDto> CreateEvent(long organizationId, long userId, CreateEventRequestDto dto, long? projectId);
    Task<List<EventResponseDto>> BulkCreateEvents(long organizationId, List<CreateEventRequestDto> events, long? projectId);
}
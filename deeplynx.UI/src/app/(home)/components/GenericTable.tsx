...
const GenericTable = <T extends object>(
  ...
  // Filter data based on the search input (only for client-side pagination)
  const filteredData = React.useMemo(() => {
    if (backendPagination) return data; // Skip filtering for backend pagination
  ...
  }, [data, columns, filterText, backendPagination]);
  ...
  const sortedData = React.useMemo(() => {
    if (backendPagination) return filteredData; // Skip sorting for backend pagination
  ...
  const currentData = backendPagination
      ? data // Backend already returns paginated data
      : enablePagination
          ? sortedData.slice(
  ...
  const showPagination = backendPagination
      ? enablePagination && paginationMetadata && paginationMetadata.totalCount > paginationMetadata.pageSize

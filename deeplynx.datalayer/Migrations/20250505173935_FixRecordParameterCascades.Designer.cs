// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using deeplynx.datalayer.Models;

#nullable disable

namespace deeplynx.datalayer.Migrations
{
    [DbContext(typeof(DeeplynxContext))]
    [Migration("20250505173935_FixRecordParameterCascades")]
    partial class FixRecordParameterCascades
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.2")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("RecordTag", b =>
                {
                    b.Property<long>("RecordId")
                        .HasColumnType("bigint")
                        .HasColumnName("record_id");

                    b.Property<long>("TagId")
                        .HasColumnType("bigint")
                        .HasColumnName("tag_id");

                    b.HasKey("RecordId", "TagId")
                        .HasName("record_tags_pkey");

                    b.HasIndex(new[] { "RecordId" }, "idx_record_tags_record_id");

                    b.HasIndex(new[] { "TagId" }, "idx_record_tags_tag_id");

                    b.ToTable("record_tags", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Class", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<string>("Description")
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<string>("Uuid")
                        .HasColumnType("text")
                        .HasColumnName("uuid");

                    b.HasKey("Id")
                        .HasName("classes_pkey");

                    b.HasIndex(new[] { "ProjectId" }, "IX_classes_project_id");

                    b.HasIndex(new[] { "Id" }, "idx_classes_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_classes_project_id");

                    b.HasIndex(new[] { "Uuid" }, "idx_classes_uuid");

                    b.ToTable("classes", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.DataSource", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Abbreviation")
                        .HasColumnType("text")
                        .HasColumnName("abbreviation");

                    b.Property<string>("BaseUri")
                        .HasColumnType("text")
                        .HasColumnName("base_uri");

                    b.Property<string>("Config")
                        .HasColumnType("jsonb")
                        .HasColumnName("config");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<string>("Type")
                        .HasColumnType("text")
                        .HasColumnName("type");

                    b.HasKey("Id")
                        .HasName("data_sources_pkey");

                    b.HasIndex(new[] { "ProjectId" }, "IX_data_sources_project_id");

                    b.HasIndex(new[] { "Id" }, "idx_data_sources_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_data_sources_project_id");

                    b.ToTable("data_sources", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Edge", b =>
                {
                    b.Property<long>("OriginId")
                        .HasColumnType("bigint")
                        .HasColumnName("origin_id");

                    b.Property<long>("DestinationId")
                        .HasColumnType("bigint")
                        .HasColumnName("destination_id");

                    b.Property<long?>("DataSourceId")
                        .HasColumnType("bigint")
                        .HasColumnName("data_source_id");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<string>("Properties")
                        .HasColumnType("jsonb")
                        .HasColumnName("properties");

                    b.Property<long?>("RelationshipId")
                        .HasColumnType("bigint")
                        .HasColumnName("relationship_id");

                    b.Property<string>("RelationshipName")
                        .HasColumnType("text")
                        .HasColumnName("relationship_name");

                    b.HasKey("OriginId", "DestinationId")
                        .HasName("edges_pkey");

                    b.HasIndex(new[] { "DestinationId" }, "IX_edges_destination_id");

                    b.HasIndex(new[] { "RelationshipId" }, "IX_edges_relationship_id");

                    b.HasIndex(new[] { "DataSourceId" }, "idx_edges_data_source_id");

                    b.HasIndex(new[] { "DestinationId" }, "idx_edges_destination_id");

                    b.HasIndex(new[] { "OriginId" }, "idx_edges_origin_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_edges_project_id");

                    b.HasIndex(new[] { "RelationshipId" }, "idx_edges_relationship_id");

                    b.ToTable("edges", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.EdgeParameter", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<long>("DestinationId")
                        .HasColumnType("bigint")
                        .HasColumnName("destination_id");

                    b.Property<string>("DestinationParams")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("destination_params");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<long>("OriginId")
                        .HasColumnType("bigint")
                        .HasColumnName("origin_id");

                    b.Property<string>("OriginParams")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("origin_params");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<long>("RelationshipId")
                        .HasColumnType("bigint")
                        .HasColumnName("relationship_id");

                    b.HasKey("Id")
                        .HasName("edge_parameters_pkey");

                    b.HasIndex(new[] { "DestinationId" }, "IX_edge_parameters_destination_id");

                    b.HasIndex(new[] { "OriginId" }, "IX_edge_parameters_origin_id");

                    b.HasIndex(new[] { "ProjectId" }, "IX_edge_parameters_project_id");

                    b.HasIndex(new[] { "RelationshipId" }, "IX_edge_parameters_relationship_id");

                    b.HasIndex(new[] { "DestinationId" }, "idx_edge_parameters_destination_id");

                    b.HasIndex(new[] { "Id" }, "idx_edge_parameters_id");

                    b.HasIndex(new[] { "OriginId" }, "idx_edge_parameters_origin_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_edge_parameters_project_id");

                    b.HasIndex(new[] { "RelationshipId" }, "idx_edge_parameters_relationship_id");

                    b.ToTable("edge_parameters", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Permission", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime?>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("permissions_pkey");

                    b.HasIndex(new[] { "Id" }, "idx_permissions_id");

                    b.ToTable("permissions", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Project", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Abbreviation")
                        .HasColumnType("text")
                        .HasColumnName("abbreviation");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("projects_pkey");

                    b.HasIndex(new[] { "Id" }, "idx_projects_id");

                    b.ToTable("projects", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Record", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long?>("ClassId")
                        .HasColumnType("bigint")
                        .HasColumnName("class_id");

                    b.Property<string>("ClassName")
                        .HasColumnType("text")
                        .HasColumnName("class_name");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<string>("CustomId")
                        .HasColumnType("text")
                        .HasColumnName("custom_id");

                    b.Property<long>("DataSourceId")
                        .HasColumnType("bigint")
                        .HasColumnName("data_source_id");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<string>("OriginalId")
                        .HasColumnType("text")
                        .HasColumnName("original_id");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<string>("Properties")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("properties");

                    b.Property<string>("Uri")
                        .HasColumnType("text")
                        .HasColumnName("uri");

                    b.HasKey("Id")
                        .HasName("records_pkey");

                    b.HasIndex(new[] { "ClassId" }, "IX_records_class_id");

                    b.HasIndex(new[] { "DataSourceId" }, "IX_records_data_source_id");

                    b.HasIndex(new[] { "ProjectId" }, "IX_records_project_id");

                    b.HasIndex(new[] { "ClassId" }, "idx_records_class_id");

                    b.HasIndex(new[] { "ClassName" }, "idx_records_class_name");

                    b.HasIndex(new[] { "DataSourceId" }, "idx_records_data_source_id");

                    b.HasIndex(new[] { "Id" }, "idx_records_id");

                    b.HasIndex(new[] { "Name" }, "idx_records_name");

                    b.HasIndex(new[] { "OriginalId" }, "idx_records_original_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_records_project_id");

                    b.HasIndex(new[] { "Properties" }, "idx_records_properties");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex(new[] { "Properties" }, "idx_records_properties"), "gin");

                    b.ToTable("records", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.RecordParameter", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long?>("ClassId")
                        .HasColumnType("bigint")
                        .HasColumnName("class_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<string>("RecordParams")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("record_params");

                    b.Property<long?>("TagId")
                        .HasColumnType("bigint")
                        .HasColumnName("tag_id");

                    b.HasKey("Id")
                        .HasName("record_parameters_pkey");

                    b.HasIndex(new[] { "ClassId" }, "IX_record_parameters_class_id");

                    b.HasIndex(new[] { "ClassId" }, "idx_record_parameters_class_id");

                    b.HasIndex(new[] { "Id" }, "idx_record_parameters_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_record_parameters_project_id");

                    b.HasIndex(new[] { "TagId" }, "idx_record_parameters_tag_id");

                    b.ToTable("record_parameters", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Relationship", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<string>("Description")
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<long>("DestinationId")
                        .HasColumnType("bigint")
                        .HasColumnName("destination_id");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<long>("OriginId")
                        .HasColumnType("bigint")
                        .HasColumnName("origin_id");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<string>("Uuid")
                        .HasColumnType("text")
                        .HasColumnName("uuid");

                    b.HasKey("Id")
                        .HasName("relationships_pkey");

                    b.HasIndex(new[] { "DestinationId" }, "IX_relationships_destination_id");

                    b.HasIndex(new[] { "OriginId" }, "IX_relationships_origin_id");

                    b.HasIndex(new[] { "ProjectId" }, "IX_relationships_project_id");

                    b.HasIndex(new[] { "DestinationId" }, "idx_relationships_destination_id");

                    b.HasIndex(new[] { "Id" }, "idx_relationships_id");

                    b.HasIndex(new[] { "OriginId" }, "idx_relationships_origin_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_relationships_project_id");

                    b.HasIndex(new[] { "Uuid" }, "idx_relationships_uuid");

                    b.ToTable("relationships", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Role", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("ModifiedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.HasKey("Id")
                        .HasName("roles_pkey");

                    b.HasIndex(new[] { "ProjectId" }, "IX_roles_project_id");

                    b.HasIndex(new[] { "Id" }, "idx_roles_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_roles_project_id");

                    b.ToTable("roles", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.RolePermission", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("ActionType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("action_type");

                    b.Property<long>("PermissionId")
                        .HasColumnType("bigint")
                        .HasColumnName("permission_id");

                    b.Property<long>("RoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("role_id");

                    b.HasKey("Id")
                        .HasName("role_permissions_pkey");

                    b.HasIndex(new[] { "ActionType" }, "idx_role_permissions_action_type");

                    b.HasIndex(new[] { "PermissionId" }, "idx_role_permissions_permission_id");

                    b.HasIndex(new[] { "RoleId" }, "idx_role_permissions_role_id");

                    b.ToTable("role_permissions", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.RoleResource", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long?>("DataSourceId")
                        .HasColumnType("bigint")
                        .HasColumnName("data_source_id");

                    b.Property<bool?>("HasAccess")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("has_access");

                    b.Property<long?>("RecordId")
                        .HasColumnType("bigint")
                        .HasColumnName("record_id");

                    b.Property<long>("RoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("role_id");

                    b.Property<long?>("TagId")
                        .HasColumnType("bigint")
                        .HasColumnName("tag_id");

                    b.HasKey("Id")
                        .HasName("role_resources_pkey");

                    b.HasIndex(new[] { "DataSourceId" }, "idx_role_resources_data_source_id");

                    b.HasIndex(new[] { "RecordId" }, "idx_role_resources_record_id");

                    b.HasIndex(new[] { "RoleId" }, "idx_role_resources_role_id");

                    b.HasIndex(new[] { "TagId" }, "idx_role_resources_tag_id");

                    b.ToTable("role_resources", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Tag", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime?>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.HasKey("Id")
                        .HasName("tags_pkey");

                    b.HasIndex(new[] { "Id" }, "idx_tags_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_tags_project_id");

                    b.ToTable("tags", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.User", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("text")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("deleted_at");

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("display_name");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("email");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp without time zone")
                        .HasColumnName("modified_at");

                    b.Property<string>("ModifiedBy")
                        .HasColumnType("text")
                        .HasColumnName("modified_by");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("type");

                    b.HasKey("Id")
                        .HasName("users_pkey");

                    b.HasIndex(new[] { "Id" }, "idx_users_id");

                    b.ToTable("users", "deeplynx");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.UserProject", b =>
                {
                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.Property<long>("ProjectId")
                        .HasColumnType("bigint")
                        .HasColumnName("project_id");

                    b.Property<long?>("RoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("role_id");

                    b.HasKey("UserId", "ProjectId")
                        .HasName("user_projects_pkey");

                    b.HasIndex(new[] { "ProjectId" }, "IX_user_projects_project_id");

                    b.HasIndex(new[] { "ProjectId" }, "idx_user_projects_project_id");

                    b.HasIndex(new[] { "RoleId" }, "idx_user_projects_role_id");

                    b.HasIndex(new[] { "UserId" }, "idx_user_projects_user_id");

                    b.ToTable("user_projects", "deeplynx");
                });

            modelBuilder.Entity("RecordTag", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Record", null)
                        .WithMany()
                        .HasForeignKey("RecordId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("record_tags_record_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Tag", null)
                        .WithMany()
                        .HasForeignKey("TagId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("record_tags_tag_id_fkey");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Class", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("Classes")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("classes_project_id_fkey");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.DataSource", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("DataSources")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("data_sources_project_id_fkey");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Edge", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.DataSource", "DataSource")
                        .WithMany("Edges")
                        .HasForeignKey("DataSourceId")
                        .HasConstraintName("edges_data_source_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Record", "Destination")
                        .WithMany("EdgeDestinations")
                        .HasForeignKey("DestinationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edges_destination_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Record", "Origin")
                        .WithMany("EdgeOrigins")
                        .HasForeignKey("OriginId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edges_origin_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("Edges")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edges_project_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Relationship", "Relationship")
                        .WithMany("Edges")
                        .HasForeignKey("RelationshipId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("edges_relationship_id_fkey");

                    b.Navigation("DataSource");

                    b.Navigation("Destination");

                    b.Navigation("Origin");

                    b.Navigation("Project");

                    b.Navigation("Relationship");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.EdgeParameter", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Class", "Destination")
                        .WithMany("EdgeParameterDestinations")
                        .HasForeignKey("DestinationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edge_parameters_destination_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Class", "Origin")
                        .WithMany("EdgeParameterOrigins")
                        .HasForeignKey("OriginId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edge_parameters_origin_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("EdgeParameters")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edge_parameters_project_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Relationship", "Relationship")
                        .WithMany("EdgeParameters")
                        .HasForeignKey("RelationshipId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("edge_parameters_relationship_id_fkey");

                    b.Navigation("Destination");

                    b.Navigation("Origin");

                    b.Navigation("Project");

                    b.Navigation("Relationship");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Record", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Class", "Class")
                        .WithMany("Records")
                        .HasForeignKey("ClassId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("records_class_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.DataSource", "DataSource")
                        .WithMany("Records")
                        .HasForeignKey("DataSourceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("records_data_source_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("Records")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("records_project_id_fkey");

                    b.Navigation("Class");

                    b.Navigation("DataSource");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.RecordParameter", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Class", "Class")
                        .WithMany("RecordParameters")
                        .HasForeignKey("ClassId")
                        .HasConstraintName("record_parameters_class_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("RecordParameters")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("record_parameters_project_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Tag", "Tag")
                        .WithMany("RecordParameters")
                        .HasForeignKey("TagId")
                        .HasConstraintName("record_parameters_tag_id_fkey");

                    b.Navigation("Class");

                    b.Navigation("Project");

                    b.Navigation("Tag");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Relationship", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Class", "Destination")
                        .WithMany("RelationshipDestinations")
                        .HasForeignKey("DestinationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("relationships_destination_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Class", "Origin")
                        .WithMany("RelationshipOrigins")
                        .HasForeignKey("OriginId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("relationships_origin_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("Relationships")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("relationships_project_id_fkey");

                    b.Navigation("Destination");

                    b.Navigation("Origin");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Role", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("Roles")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("roles_project_id_fkey");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.RolePermission", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Permission", "Permission")
                        .WithMany("RolePermissions")
                        .HasForeignKey("PermissionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("role_permissions_permission_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Role", "Role")
                        .WithMany("RolePermissions")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("role_permissions_role_id_fkey");

                    b.Navigation("Permission");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.RoleResource", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.DataSource", "DataSource")
                        .WithMany("RoleResources")
                        .HasForeignKey("DataSourceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("role_resources_data_source_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Record", "Record")
                        .WithMany("RoleResources")
                        .HasForeignKey("RecordId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("role_resources_record_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Role", "Role")
                        .WithMany("RoleResources")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("role_resources_role_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Tag", "Tag")
                        .WithMany("RoleResources")
                        .HasForeignKey("TagId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("role_resources_tag_id_fkey");

                    b.Navigation("DataSource");

                    b.Navigation("Record");

                    b.Navigation("Role");

                    b.Navigation("Tag");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Tag", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("Tags")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("tags_project_id_fkey");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.UserProject", b =>
                {
                    b.HasOne("deeplynx.datalayer.Models.Project", "Project")
                        .WithMany("UserProjects")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("user_projects_project_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.Role", "Role")
                        .WithMany("UserProjects")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("user_projects_role_id_fkey");

                    b.HasOne("deeplynx.datalayer.Models.User", "User")
                        .WithMany("UserProjects")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("user_projects_user_id_fkey");

                    b.Navigation("Project");

                    b.Navigation("Role");

                    b.Navigation("User");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Class", b =>
                {
                    b.Navigation("EdgeParameterDestinations");

                    b.Navigation("EdgeParameterOrigins");

                    b.Navigation("RecordParameters");

                    b.Navigation("Records");

                    b.Navigation("RelationshipDestinations");

                    b.Navigation("RelationshipOrigins");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.DataSource", b =>
                {
                    b.Navigation("Edges");

                    b.Navigation("Records");

                    b.Navigation("RoleResources");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Permission", b =>
                {
                    b.Navigation("RolePermissions");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Project", b =>
                {
                    b.Navigation("Classes");

                    b.Navigation("DataSources");

                    b.Navigation("EdgeParameters");

                    b.Navigation("Edges");

                    b.Navigation("RecordParameters");

                    b.Navigation("Records");

                    b.Navigation("Relationships");

                    b.Navigation("Roles");

                    b.Navigation("Tags");

                    b.Navigation("UserProjects");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Record", b =>
                {
                    b.Navigation("EdgeDestinations");

                    b.Navigation("EdgeOrigins");

                    b.Navigation("RoleResources");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Relationship", b =>
                {
                    b.Navigation("EdgeParameters");

                    b.Navigation("Edges");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Role", b =>
                {
                    b.Navigation("RolePermissions");

                    b.Navigation("RoleResources");

                    b.Navigation("UserProjects");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.Tag", b =>
                {
                    b.Navigation("RecordParameters");

                    b.Navigation("RoleResources");
                });

            modelBuilder.Entity("deeplynx.datalayer.Models.User", b =>
                {
                    b.Navigation("UserProjects");
                });
#pragma warning restore 612, 618
        }
    }
}
